...
public class RunningAppView {
  ...
  public static String getMenuText() {
    return "1. [Easy: Sammi Liu, Marcus Robertson] List all routes\n"
        + "2. [Easy: Sammi Liu, Marcus Robertson, Ashley Pupkin] List the friends of a user\n"
        + "3. [Easy: Ashley Pupkin] Print all achievements\n"
        + "4. [Hard: Ashley Pupkin] List saved routes by date added\n"
  ...
  public static void displayRoutes(List<Routes> routes) { //Sammi & Marcus
      ...
      System.out.printf("%s\n", "-".repeat(LINE_WIDTH));
      System.out.printf("%-25s %-25s %-8s %-6s %-10s\n",
              "startLocation", "endLocation", "routeID", "accID", "distancePreference");

      for (Routes route : routes) {
          System.out.printf("%-25s %-25s %-8s %-6s %-10s\n",
                  route.getStartLocation(), route.getEndLocation(),
                  route.getRouteID(), route.getAccID(),
                  route.getDistancePreference());
      }
...
public static void displayTotalMiles(double miles){//Ashley
  ...
  */
    ...
    public static void displayEndingRoutes(List<Routes> routes) { //Sammi & Marcus
        System.out.printf("%s\n", "-".repeat(70));
        System.out.printf("%-25s %-25s %-10s %-10s %-20s\n", "endLocation", "startLocation", "routeID", "accID",
            "distancePreference");
        for (Routes route : routes) {
            System.out.printf("%-25s %-25s %-10s %-10s %-20s\n", route.getEndLocation(), route.getStartLocation(),
                route.getRouteID(), route.getAccID(), route.getDistancePreference());
    }
    System.out.printf("%s\n", "-".repeat(70));
 }
  /**
...
/**
  ...
  public static void displayOrderedRoutes(List<Routes> orderedRoutes) { //Sammi & Marcus
    System.out.printf("%s\n", "-".repeat(LINE_WIDTH));
    System.out.printf("%-22s %-4s %-3s %-5s %-9s\n", "startLocation", "endLocation", "routeID", "accID",
         "distancePreference");
